...
static ili9341_bool_t ili9341_clip_rect(ili9341_t *lcd, int16_t *x, int16_t *y, uint16_t *w, uint16_t *h);
static void ili9341_fill_quarter_circle(ili9341_t *lcd, ili9341_color_t color, int16_t x, int16_t y, int16_t r, uint8_t corners, int16_t delta);

// ------------------------------------------------------- exported functions --

void ili9341_spi_tft_set_address_rect(ili9341_t *lcd, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
	ili9341_spi_tft_select(lcd);

	// column address set
	ili9341_spi_write_command_data(lcd, issNONE, 0x2A, 4, (uint8_t[] ) { (x0 >> 8) & 0xFF, x0 & 0xFF, (x1 >> 8) & 0xFF, x1 & 0xFF });

	// row address set
	ili9341_spi_write_command_data(lcd, issNONE, 0x2B, 4, (uint8_t[] ) { (y0 >> 8) & 0xFF, y0 & 0xFF, (y1 >> 8) & 0xFF, y1 & 0xFF });
...
void ili9341_draw_pixel(ili9341_t *lcd, ili9341_color_t color, int16_t x, int16_t y) {
...
void ili9341_draw_line(ili9341_t *lcd, ili9341_color_t color, int16_t x0, int16_t y0, int16_t x1, int16_t y1) {
	int16_t dx = x1 - x0;
	int16_t dy = y1 - y0;
	...
	while (x0 <= x1) {
		...
		if ((x >= 0) && (x <= lcd->screen_size.width) && (y >= 0) && (y <= lcd->screen_size.height)) {
...
void ili9341_draw_rect(ili9341_t *lcd, ili9341_color_t color, int16_t x, int16_t y, uint16_t w, uint16_t h) {
...
void ili9341_fill_rect(ili9341_t *lcd, ili9341_color_t color, int16_t x, int16_t y, uint16_t w, uint16_t h) {
...
void ili9341_draw_circle(ili9341_t *lcd, ili9341_color_t color, int16_t x, int16_t y, int16_t r) {
	int16_t f = 1 - r;
	int16_t fx = 1;
	int16_t fy = -2 * r;
	int16_t ix = 0;
...
void ili9341_fill_circle(ili9341_t *lcd, ili9341_color_t color, int16_t x, int16_t y, int16_t r) {
...
void ili9341_fill_screen(ili9341_t *lcd, ili9341_color_t color) {
	ili9341_fill_rect(lcd, color, 0, 0, lcd->screen_size.width, lcd->screen_size.height);
}

void ili9341_draw_bmp(ili9341_t *lcd, int start_x, int start_y, int w, int h, const uint16_t bmp[]) {
	uint16_t tx_row[lcd->screen_size.width];
	int x, y, pos = 0;
	for (y = 0; y < h; y++) {
		for (x = 0; x < w; x++) {
			ili9341_color_t color = bmp[pos++];
			uint16_t color_le = __LEu16(&color);
			tx_row[x] = color_le;
			//ili9341_draw_pixel(lcd, color, x, y);
		}

		ili9341_spi_tft_set_address_rect(lcd, start_x, start_y+y, w + start_x, y + start_y + 1);
		ili9341_spi_tft_select(lcd);
		HAL_GPIO_WritePin(lcd->data_command_port, lcd->data_command_pin, __GPIO_PIN_SET__);
		ili9341_transmit_color(lcd, w * 2, tx_row, ibYes);
		ili9341_spi_tft_release(lcd);
...
//	while (p < px) {
//
////		ili9341_color_t color= ili9341_color_wheel(bmp[p]);
//		if (x > end_x) {
//			x = start_x;
//			y++;
//		} else {
...
//		p++;
//	}
//	uint16_t data = test_img_128x128[x][y];
//
//	ili9341_spi_tft_select(lcd);
//
//	HAL_GPIO_WritePin(lcd->data_command_port, lcd->data_command_pin, __GPIO_PIN_SET__);
//	HAL_SPI_Transmit(lcd->spi_hal, bmp, sizeof(uint16_t) * end_x * end_y, __SPI_MAX_DELAY__);
//	ili9341_spi_tft_release(lcd);

	//}
}

void ili9341_draw_bitmap_1b(ili9341_t *lcd, ili9341_color_t fg_color, ili9341_color_t bg_color, int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t *bmp) {
...
void ili9341_draw_char(ili9341_t *lcd, ili9341_text_attr_t attr, char ch) {
	...
	// select target region
	ili9341_spi_tft_set_address_rect(lcd, attr.origin_x, attr.origin_y, attr.origin_x + attr.font->width - 1, attr.origin_y + attr.font->height - 1);
	ili9341_spi_tft_select(lcd);
...
void ili9341_draw_string(ili9341_t *lcd, ili9341_text_attr_t attr, char str[]) {
	...
	while ('\0' != *str) {
		if ('\r' == *str) {
			...
			if ((curr_x + attr.font->width) > lcd->screen_size.width) {
				curr_x = 0;
				curr_y += attr.font->height;
